"""
the_booted_hunt.py contains the training algorihtm function for leanring the Sinkhorn centered logarithms of Sinkhorn scale factors
using the bootstrapping method for training.
"""
...
from utils import test_sampler, prior_sampler, rando, random_shapes_loader, rn_rs
from test_funcs import test_pred_dist, test_pred_loss

# Training algorithm
def the_booted_hunt(gen_net,
             pred_net,
             C,
             reg,
             dim_prior,
  ...
  for i in tqdm(range(n_samples//batchsize)):
    ...
    for e in range(epochs):
      ...
      for j in range(batchsize//minibatch):
        X_mini = X_e[j*minibatch:(j+1)*minibatch]     
        P_mini = pred_net(X_mini)

        # Target creation
        with torch.no_grad():
          V0 = torch.exp(P_mini)
          V = sink_vec(X_mini[:, :dim], X_mini[:, dim:], C, reg, V0, boot_no)
          V = torch.log(V)
